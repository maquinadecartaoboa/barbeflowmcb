...
export function TopClientesCard({ clientes }: { clientes: TopClienteRow[] }) {
  return (
    <Card>
...
// Stable color per source_key from the RPC. Unknown keys fall back to muted.
const MIX_PALETTE: Record<string, { bar: string; label: string }> = {
  booking_service:  { bar: "bg-primary",        label: "text-primary" },
  subscription:     { bar: "bg-violet-500",     label: "text-violet-300" },
  booking_product:  { bar: "bg-cyan-500",       label: "text-cyan-300" },
  booking:          { bar: "bg-sky-500",        label: "text-sky-300" },
  package:          { bar: "bg-emerald-500",    label: "text-emerald-300" },
  courtesy:         { bar: "bg-pink-500",       label: "text-pink-300" },
  other:            { bar: "bg-zinc-500",       label: "text-zinc-300" },
};

function mixColor(key: string) {
  return MIX_PALETTE[key] ?? MIX_PALETTE.other;
}

export function MixReceitaCard({ items }: { items: MixReceitaItem[] }) {
            ...
            return (
              <div key={it.source_key} className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className={cn("font-medium truncate pr-2", palette.label)}>{it.source}</span>
...
interface AniversariantesCompactProps {
    ...
    <Card>
      ...
      <CardContent className="space-y-1.5">
        {visible.map((b) => {
          ...
          const labelClass = b.days_until === 0
        ...
        {hidden > 0 && !expanded && (
          ...
          >
            Ver toda a semana ({hidden} a mais)
          </Button>
        )}
